...
(function($){
    ...
    $.addFlex = function(t,p)
    {
        ...
        var g = {
                        ...
                        function(i,row)
                        {
                            ...
                            (
                                function ()
                                {
                                    ...
                                    // jdw: Changed to fetch using name from colModel
                                    var model = p.colModel[idx];
                                    td.align = this.align;
                                    td.innerHTML = row[model.name];
                                    $(tr).append(td);
                                    td = null;
                                }
                                );
            ...
            // jdw: Added so that a hash of the row can be generated for use in callbacks
            getRowData: function(row) {
                var data = {};

                $('td', row).each(function(index) {
                    var model = p.colModel[index];
                    data[model['name']] = $(this).text();
                });
                    ...
                    function ()
                    {
                        ...
                        .click(
                            function (e)
                            {
                                ...
                                // jdw: Added on click handler
                                if(p.onClick)
                                    p.onClick({
                                        data: table.getRowData(this)
                                    });
                            }
                            )
                        .mousedown(
        ...
        (
            function ()
            {
                ...
                // jdw: fetch the column model
                var model = p.colModel[ci];
                ...
                // jdw: If the model width is auto then add it to the audoWidths
                // array. Otherwise set as normal and increment the width counter
                if(model.width != 'auto') {
                    $(thdiv).css({
        ...
        // FIXME: constant 11!
        tw = $(g.hDiv).width() - (tw+(11*p.colModel.length));
        // Apply width to each auto col
        autoWidths.forEach(function(div){
            $(div).width(tw/autoWidths.length);
        });
